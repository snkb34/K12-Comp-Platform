...
def is_licensed_source(source: Source) -> bool:
    group = (source.employee_group or source.category or '').strip().lower()
    return group in {'licensed', 'teacher', 'teachers', 'certified'}
...
def run_update(db: Session):
    ...
    run.message = (
        ...
        f'and extracted {total_rows} records.'
    )
